...
@app.get("/make_image3/{issue}/{deta}/{place}/{race_number}/{money}/{vote_type}/{horse_number1}/{horse_number2}/{horse_number3}")
async def make_image(issue: int, deta: str, place: str, race_number: int, money: int, vote_type: int, horse_number1: int, horse_number2: int, horse_number3: int):
    json_data = {
        "issue_date": issue,
    ...
    print(json_data)
    json_str = json.dumps(json_data)
    return FileResponse(json2_image.make_image(json_str))
